...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Fetch the current staff data to compare old password
    $ch = curl_init($apiUrl);
    ...
    if (!curl_errno($ch)) {
        ...
        if (isset($result['password'])) {
            ...
            if ($oldPassword !== $result['password']) {
                ...
                $jsonData = json_encode($staffData);
                ...
                if (curl_errno($ch)) {
                    ...
                    if (isset($result['message'])) {
                        if (strpos(strtolower($result['message']), 'staff updated successfully!') !== false) {
    ...
    // Fetch current staff data if the form is not submitted yet
    $ch = curl_init($apiUrl);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
    $response = curl_exec($ch);
